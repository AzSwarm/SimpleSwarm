...
namespace SimpleSwarm
{
    ...
    public class AddSimpleClusterWorkerCmdletCommand : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            string filter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, "Manager");
            TableQuery<SimpleSwarmVM> tableQuery = new TableQuery<SimpleSwarmVM>().Where(filter).Select(new List<string> { });

            String managerVM = "";
            foreach (SimpleSwarmVM entity in table.ExecuteQuery(tableQuery))
            {
                managerVM = entity.RowKey;
            }
            IVirtualMachine virtualMachineManager = azure.VirtualMachines.GetByResourceGroup(ResourceGroupName, managerVM);
            String managerIp = virtualMachineManager.GetPrimaryNetworkInterface().PrimaryPrivateIP;
            ...
            cloudInit = cloudInit.Replace("<swarmManagerIp>", managerIp);
            cloudInitBase64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(cloudInit));

            IAvailabilitySet availabilitySet = azure.AvailabilitySets.GetByResourceGroup(resourceGroupName, "azswarm-worker-avset");
            ...
            table = tableClient.GetTableReference("SimpleSwarmSetup");
